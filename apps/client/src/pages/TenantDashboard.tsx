  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([t, tenantContracts, allPendingReceipts]) => {
        ...
        const tenantReceipts = allPendingReceipts.filter((r) =>
      ...
      .catch((err) => {
        console.error(err);
        showError('Error al cargar datos del inquilino');
      })
    ...
    return (
      <div className="animate-fade-in">
        <EmptyState
          ...
          description="El inquilino solicitado no existe."
        />
  ...
  return (
    <div className="animate-fade-in max-w-5xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <Link
          to={
            activeContract?.department?.property
              ? `/properties/${activeContract.department.property.id}`
              : '/tenants'
          }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <section>
            ...
            {activeContract ? (
              <div className="bg-surface rounded-2xl border border-primary-100/80 shadow-sm p-4 sm:p-6 relative overflow-hidden">
                ...
                <div className="mb-4">
                  <h3 className="text-xl font-bold text-on-surface tracking-tight">
                    {activeContract.department?.name || 'Departamento'}
                  </h3>
                  <p className="text-on-surface-muted text-[13px]">
                    {activeContract.department?.property?.name} -{' '}
                    {activeContract.department?.property?.address}
                  </p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  <div className="p-3 rounded-xl bg-surface-alt/80 ring-1 ring-border-ring">
                    <p className="text-on-surface-muted text-[11px] mb-1 uppercase tracking-wider font-medium">
                      Periodo
                    </p>
              ...
              <div className="bg-surface-alt rounded-2xl border border-border p-6 text-center">
                <p className="text-on-surface-muted text-sm">
                  Este inquilino no tiene un contrato activo actualmente.
                </p>
              </div>
            )}
          </section>
          ...
          <section>
            ...
            {pendingReceipts.length === 0 ? (
              <EmptyState
                ...
                description="No hay recibos pendientes de revision para este inquilino."
              />
            ) : (
              <>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-2 text-sm mb-3">
                        <div className="bg-surface-alt/80 rounded-lg p-2 ring-1 ring-border-ring">
                          <p className="text-on-surface-muted text-[11px] mb-0.5">
                            Total
                          </p>
                        ...
                        <div className="bg-red-50 dark:bg-red-950/40 rounded-lg p-2 ring-1 ring-red-200/50 dark:ring-red-700/40">
                          <p className="text-red-700 dark:text-red-300 text-[11px] mb-0.5">
                            Debe
                          </p>
                          <p className="font-bold text-red-600 dark:text-red-400">
                            S/ {Math.abs(receipt.balance).toFixed(2)}
                          </p>
                        </div>
                      </div>
            ...
            <section>
              ...
              <div className="space-y-3">
                {pastContracts.map((c) => (
                  <div
                    key={c.id}
                    className="bg-surface rounded-xl border border-border p-4 opacity-75 hover:opacity-100 transition-all duration-200"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-xs text-on-surface-muted">
                          {c.department?.property?.name}
                        </p>
                      </div>
                      <span className="text-[11px] px-2 py-1 bg-surface-raised text-on-surface-medium rounded-lg font-medium">
                        Finalizado
                      </span>
                    </div>
                    <div className="text-sm text-on-surface-medium flex gap-4">
                      <span className="flex items-center gap-1.5">
                        <Calendar size={13} /> {formatDate(c.startDate)} -{' '}
              ...
              <div className="bg-surface rounded-2xl border border-border overflow-hidden shadow-sm">
                <div className="divide-y divide-border-light">
                  {payments.map((p) => (
                    <div
                      key={p.id}
                      className="p-4 hover:bg-surface-alt/50 transition-colors"
                    >
