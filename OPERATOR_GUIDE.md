...
- [H+. Payment Reports (PDF)](#h-step-by-step-payment-reports-pdf)
  - [H+.1 Where to find it](#h1-where-to-find-it)
  - [H+.2 Filters](#h2-filters)
  - [H+.3 What the report contains](#h3-what-the-report-contains)
  - [H+.4 Constraints](#h4-constraints-1)
...
| **Generar reporte**             | Download a PDF payment report for a contract. Filters by date range and payment method. Grouped totals by method, net received, and refunds.                                                                                     | —                                                                      |
...
3. If you don't → click **Regístrate**. Fill the form (password must be **at least 8 characters**) and submit. You'll see _"Account pending approval"_.
4. **Wait.** Your account starts as `Pendiente`. An admin must approve you before you can log in. Until then, every login attempt will show _"Your account is pending admin approval"_.
...
| :---------------------------------------- | :------------------------------------------------------------------------------------------------------------------------- |
| You try to log in with the wrong password | Error: _"Invalid credentials"_.                                                                                            |
| Your account is still `Pendiente`         | 403: _"pending admin approval"_. Wait for an admin.                                                                        |
| Your account was `Rechazado`              | 403: _"Your account has been rejected"_. Contact the admin.                                                                |
| Email already registered                  | 409: _"Email already in use"_. Recover the password offline.                                                               |
...
A _Propiedad_ is the building you own. It owns the **electricity rate** (`S/ / kWh`) and **water rate** (`S/ / m³`) that all its units inherit.
...
A _Departamento_ is one rentable space inside a property.
...
An _Inquilino_ is the actual person renting. They are **not** a system user — they don't log in.
...
The _Contrato_ binds one tenant to one department for a date range and captures the financial terms.
...
| Scenario                                                  | What happens                                                                                                                                                                                                      |
| :-------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The department is already rented (has an active contract) | Blocked: _"Department X is not available for rent"_.                                                                                                                                                              |
...
If any request fails, you'll see the toast _"No se pudieron cargar los datos de facturacion"_.
...
| :-------------- | :-------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `unpaid`        | none            | Upserts. Status stays `unpaid`. Idempotent — click as many times as you want; the receipt picks up the latest readings, extras, and date-based payments. **Also refreshes the `carryForwardDetails` snapshot** to reflect any payments made against _previous_ months since the last generation. |
| `unpaid`        | one or more     | **409 `RECEIPT_HAS_PAYMENTS`**: _"Receipt for M/Y has N linked payment(s); delete them before regenerating."_ You must delete the payments (from the Pagos page) before regenerating, then re-record them after.                                                                                 |
| `paid`          | any             | **409 `RECEIPT_LOCKED`**: _"Receipt for M/Y is paid and cannot be regenerated."_ The UI also disables the Regenerar Recibo button with hover tooltip _"Recibo pagado — no se puede regenerar"_.                                                                                                  |
...
| `carryForwardDetails`     | jsonb array of `{period, balance}`     | Snapshotted at issue time                                                                                                                                                       | A list of unpaid debts from months _prior_ to the receipt month. Used to render the "Saldo anterior" section.                                                                              |
...
## H+. Payment Reports (PDF)

PropManager can generate a downloadable PDF report of all payments for a single contract. This is useful for reconciliation, record-keeping, or sharing with the tenant.

### H+.1 Where to find it

There are **two entry points**:

1. **Sidebar → Pagos → "Generar reporte" button** (in the page header, next to "Nuevo Pago"). Opens a modal where you pick the contract and optional filters.
2. **Department billing page → download icon button** (next to "Generar Recibo"). Opens the same modal with the current contract **locked** (cannot be changed).

### H+.2 Filters

All filters are optional. Leaving them blank returns **all payments** on the selected contract.

| Filter   | Type     | Notes                                                                                              |
| :------- | :------- | :------------------------------------------------------------------------------------------------- |
| Contrato | required | The contract to report on. Pre-filled and locked when launched from the billing page.              |
| Desde    | optional | Start date (inclusive). Payments on or after this date are included. Format: YYYY-MM-DD.           |
| Hasta    | optional | End date (inclusive). Payments on or before this date are included. Cannot be in the future.       |
| Método   | optional | Filter to a single payment method (Efectivo, Transferencia, Yape, Plin, Otro). "Todos" = no filter. |

**Validation:** if both dates are set, `Desde` must be ≤ `Hasta`. If `Hasta` is in the future, the submit button is disabled.

### H+.3 What the report contains

The downloaded PDF includes:

| Section             | Details                                                                                                    |
| :------------------ | :--------------------------------------------------------------------------------------------------------- |
| Header              | "REPORTE DE PAGOS" title, generation timestamp, property address, department name, tenant name, contract dates. |
| Filters applied     | Shows the date range and method filter if any were set.                                                     |
| Payment table       | One row per payment: Date, Method, Reference, Receipt period (or "Sin recibo"), Notes, Amount. Negative amounts (refunds) appear in red with a leading minus sign. Ordered by date ascending. |
| Subtotals by method | Sum of payments grouped by payment method (only methods with non-zero totals shown).                       |
| Refunds             | Sum of all negative payments.                                                                              |
| Total recibido      | Net total (gross — refunds are already included since they are negative).                                  |
| Footer              | "Generado: <date> por <operator email>".                                                                   |

**Multi-page:** if a contract has many payments, the report continues across multiple A4 pages with column headers repeated on each page.

### H+.4 Constraints

| Scenario                                                  | What happens                                                                                     |
| :-------------------------------------------------------- | :----------------------------------------------------------------------------------------------- |
| The contract does not exist                               | Error: `CONTRACT_NOT_FOUND`. Modal shows "Contrato no encontrado".                               |
| `Hasta` is in the future                                  | Submit is disabled in the UI. If bypassed via API, backend returns `400 INVALID_DATE_RANGE`.     |
| `Desde` > `Hasta`                                         | Submit is disabled in the UI. Backend returns `400 INVALID_DATE_RANGE`.                          |
| No payments match the filters                              | A PDF is still generated showing "Total recibido: S/ 0.00" with empty rows.                      |
| The contract has 0 payments at all                         | Same as above — a valid 1-page PDF is downloaded.                                                |
...
| Scenario                                                 | What happens                                                                                                                                                                                                                                                                                                                                               |
| :------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Contract is already `Terminado`                          | Blocked: 409 _"Contract is already terminated"_. View the existing termination snapshot instead.                                                                                                                                                                                                                                                           |
...
| **Download a payment report** with a future `Hasta` date                    | UI blocks submit; backend returns `400 INVALID_DATE_RANGE`                                                               | Use today or a past date.                                                                                                                                                                                                                                                                                     |
...
**Q: Can I export payments as a PDF report?**
A: Yes. Click **"Generar reporte"** on the Pagos page (or the download icon on the billing page). Choose the contract and optional date/method filters, then click "Descargar PDF". The report streams immediately (no async queue like receipt PDFs). See section H+ for full details.
...
| **Billing Period**                | The (month, year) tuple a meter reading is attributed to. A day-1-of-month reading is attributed to the _previous_ month.                                       |
...
| **Reporte de pagos**              | PDF report of all payments for a contract, with optional date/method filters. Downloaded directly (no storage queue).                                            |
